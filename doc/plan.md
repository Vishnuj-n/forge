...
Principles:
- Safety: `forge test` uses temp isolation; `forge init` validates target before direct execution
...
Forge uses a **dual execution model**:

```
forge init: Validate Target -> Execute in Target -> Copy -> Append
forge test: Create Temp -> Execute in Temp -> Copy -> Append -> Inspect
```

`forge init` modifies the target directly. `forge test` does not modify user targets.
...
* `forge test` runs commands and file operations in an OS-provided temporary directory
* Failures are contained during testing
* `forge init` is direct-to-target after emptiness validation

### 6.2 Direct-Write + Test-Isolation

* `forge init`: execute and apply files directly in the target directory
* `forge test`: execute in temp and never commit to target
...
* Runs workflow directly in target directory
* No commit phase
...
| Partial project creation    | Empty target validation + fail-fast     |
